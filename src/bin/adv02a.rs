...
fn main() {
    let direction_counter: HashMap<Direction, i32> = include_str!("../../resources/input02.txt")
        ...
        .fold(HashMap::<Direction, i32>::new(), |mut acc, DirectionLine(direction, n)| {
            *acc.entry(direction).or_insert(0) += n; 
            acc
        });
    let total_down = direction_counter.get(&Direction::Down).unwrap_or(&0) - direction_counter.get(&Direction::Up).unwrap_or(&0);
